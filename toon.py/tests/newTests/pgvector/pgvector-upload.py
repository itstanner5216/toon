...
try:
    from chunker_ast import build_chunk_representations, chunk_file_ast, prepend_context_header
    ...
    def prepend_context_header(
        ...
        if filepath and (filepath.startswith(
                "/") or (len(filepath) >= 2 and filepath[1] == ":")):
    ...
    def build_chunk_representations(
        ...
        with_header = prepend_context_header(
            chunk_text, doc_path, symbol, language)
        return {
            "raw": headerless,
            "semantic": headerless,
            "lexical": with_header,
...
CODE_EXTENSIONS = {e.strip().lower()
                   for e in _CFG["CODE_EXTENSIONS"].split(",") if e.strip()}
CONFIG_EXTENSIONS = {
    e.strip().lower() for e in _CFG["CONFIG_EXTENSIONS"].split(",") if e.strip()
}
...
def _progress_bar(
        ...
        label: str = "") -> str:
    """Visual progress bar."""
    pct = 1.0 if total == 0 else min(current / total, 1.0)
...
def _size_fmt(num_bytes: float) -> str:
    ...
    for unit in ("B", "KB", "MB", "GB"):
        if abs(num_bytes) < 1024.0:
            return f"{
                num_bytes:.1f}{unit}" if unit != "B" else f"{
                num_bytes:.0f}{unit}"
        num_bytes /= 1024.0
...
class RateLimiter:
    ...
    def _current_delay(self) -> float:
        ...
        if escalated:
            return (
                RATE_LIMIT_ESCALATED_INITIAL if in_warmup else RATE_LIMIT_ESCALATED_SUSTAINED)
    ...
    def report_rate_limit(self) -> None:
        with self._lock:
            ...
            if hits >= RATE_LIMIT_COOLDOWN_AFTER:
                ...
                safe_print(
                    f"\n  {YELLOW}⚠  {hits} rate limits hit — taking " f"{
                        RATE_LIMIT_COOLDOWN_SECS:.0f}s cooldown break...{RESET}\n")
        ...
        with self._lock:
            if self._aborted:
                raise RateLimitAbort(
                    "Too many rate limits — operation cancelled")
            ...
            if len(self._call_times) >= RATE_MAX_PER_MINUTE:
                ...
                if wait > 0:
                    safe_print(
                        f"  {YELLOW}⏳ Minute cap ({RATE_MAX_PER_MINUTE}/min), pausing {wait:.0f}s...{RESET}"
                    )
                    ...
                    now = time.monotonic()
                    self._call_times = [
                        t for t in self._call_times if t > now - 60.0]
...
def get_embeddings_batch(
        texts: list[str], retries: int | None = None) -> list[list[float]]:
    ...
    for attempt in range(retries):
        ...
        try:
            ...
            if e.response.status_code == 429 and attempt < retries - 1:
                ...
                wait = 2 ** (attempt + 1)
                safe_print(
                    f"  {YELLOW}⏳ Rate limited, retrying in {wait}s...{RESET}")
                time.sleep(wait)
            else:
                raise
            ...
            if attempt < retries - 1:
                ...
                safe_print(
                    f"  {YELLOW}⏳ Embedding call failed ({e}), retry {
                        attempt + 1}/{retries} in {wait}s...{RESET}")
                time.sleep(wait)
            else:
                raise
    raise RateLimitAbort("Too many rate limits — operation cancelled")
    ...
    for attempt in range(retries):
        try:
            ...
            if attempt < retries - 1:
                wait = min(RETRY_BACKOFF_BASE**attempt, RETRY_BACKOFF_MAX)
                safe_print(
                    f"  {YELLOW}⏳ Store batch failed ({e}), retry {
                        attempt + 1}/{retries} in {wait}s...{RESET}")
                time.sleep(wait)
            else:
                raise
...
def get_store_sources(
    store_id: str, project_id: str | None = None
    ...
    try:
        ...
        try:
            ...
            msg = body
        print(
            f"  {RED}ERROR: GET sources failed ({e.response.status_code}): {msg[:300]}{RESET}",
            file=sys.stderr,
        )
        sys.exit(1)
    except (httpx.RequestError, ConnectionError, OSError) as e:
        ...
        print(
            f"  {DIM}Is pgvector-api running at {API_BASE}?{RESET}",
            file=sys.stderr)
        sys.exit(1)
...
def delete_store_doc(store_id: str, doc_path: str) -> int:
    ...
    try:
        ...
        safe_print(
            f"  {RED}ERROR: DELETE failed for {doc_path}: {
                e.response.status_code}{RESET}")
        return 0
...
def chunk_code_records(
    ...
    for index, line in enumerate(lines, start=1):
        ...
        is_new_symbol = (
            bool(current_lines) and symbol_name is not None and _indent_level(line) == 0)
        at_size_limit = bool(current_lines) and len(
            projected_text) > CODE_CHUNK_MAX_CHARS
        at_soft_limit = (
            bool(current_lines)
        ...
        if is_new_symbol or at_size_limit or at_soft_limit:
            if at_size_limit and not is_new_symbol and len(current_lines) > 3:
                search_start = max(
                    0, len(current_lines) - len(current_lines) // 3)
                best_break = None
...
def _split_oversized_block(
    ...
    for line in block_lines:
        ...
        if len(projected) > CODE_CHUNK_MAX_CHARS and sub_lines:
            ...
            for j in range(len(sub_lines) - 1,
                           max(0, len(sub_lines) // 2) - 1, -1):
            ...
            emit = sub_lines[:break_at] if break_at < len(
                sub_lines) else sub_lines
            remainder = sub_lines[break_at:] if break_at < len(
                sub_lines) else []
...
def chunk_config_records(
    ...
    def flush(end_line: int) -> None:
        ...
        if len(chunk_text) > CONFIG_CHUNK_MAX_CHARS:
            for i in range(0, len(chunk_text), CONFIG_CHUNK_MAX_CHARS):
                records.append(
                    _build_chunk_record(
                        chunk_text[i: i + CONFIG_CHUNK_MAX_CHARS],
                        start_line,
                        end_line,
                        {
                            **base_metadata,
    ...
    for index, line in enumerate(lines, start=1):
        stripped = line.strip()
        is_section = bool(re.match(r"^\[.*\]$", stripped)) or bool(
...
def paragraph_records(
    ...
    # Pre-expand any single line longer than MAX_CHUNK_CHARS into multiple
    # lines
...
def _apply_headers(
    ...
    Transforms each record from {"text": str, "metadata": dict} shape to
    {"semantic_content": str, "lexical_content": str, "display_content":
    str, "metadata": dict}.
    """
    ...
    for record in records:
        ...
        meta: dict[str, object] = dict(
            cast("dict[str, object]", raw_meta)) if raw_meta else {}
        symbol: str | None = str(
            meta.get("symbol_name")) if meta.get("symbol_name") else None
...
def build_chunk_records(
    ...
    if content_type == "code":
        if _AST_CHUNKER_AVAILABLE:
            ast_language = _AST_EXT_REMAP.get(
                extension, base_metadata.get("language", ""))
        ...
        if len(embeddings) != len(chunk_records):
            raise ValueError(
                f"embedding count mismatch: expected {
                    len(chunk_records)}, got {
                    len(embeddings)}")
        items: list[dict[str, object]] = []
        for chunk_record, emb, idx in zip(
            chunk_records, embeddings, batch_indices, strict=False
        ):
            meta: dict[str, object] = dict(
                cast("dict[str, object]", chunk_record.get("metadata") or {})
            )
            ...
            meta.setdefault("chunk_chars", len(str(chunk_record.get(
                "semantic_content", chunk_record.get("text", "")))), )
            meta["embedding_model"] = EMBEDDING_MODEL

            items.append(
                {
                    "content": str(
                        chunk_record.get("display_content", chunk_record.get("text", ""))
                    ),
...
class ProgressTracker:
    ...
    def __init__(
            self,
            total_files: int,
            total_chunks_estimate: int = 0) -> None:
        self._lock = threading.Lock()
        ...
        with self._lock:
            ...
            if errors:
                ...
                self.failed_files.append(
                    (rel_path, ok_chunks, total_chunks, errors))
            elif total_chunks == 0:
...
def ingest_file(
    ...
    Returns (ok_chunks, total_chunks, errors).
    """
    if display_name:
        fname = display_name
    elif repo_root:
    ...
    try:
        ...
        chunks = build_chunk_records(
            path,
            fname,
            repo_root or os.path.dirname(path),
        ...
        if not quiet:
            safe_print(
                f"    {
                    _status_icon('fail')} failed to read/chunk — {DIM}{err_msg}{RESET}")
        return 0, 0, [err_msg]
    ...
    if len(batches) == 1:
        for idx, success, err in _process_batch(
            batches[0][0], batches[0][1], len(chunks), fname, store_id
        ):
            if success:
                ok += 1
                if not quiet and not compact:
                    safe_print(
                        f"    {_status_icon('ok')} chunk {idx + 1}/{len(chunks)} ({len(str(chunks[idx]['text']))} chars)"
                    )
            elif err == "aborted":
                errors.append(err)
                if not quiet:
                    safe_print(
                        f"    {_status_icon('abort')} chunk {idx + 1}/{len(chunks)} — skipped (aborted)"
                    )
            else:
                errors.append(err)
                if not quiet:
                    safe_print(
                        f"    {_status_icon('fail')} chunk {idx + 1}/{len(chunks)} — {DIM}{err}{RESET}"
                    )
    else:
...
def _print_header(
    ...
    rate_info = f"{RATE_MAX_PER_MINUTE}/min, {RATE_DELAY_INITIAL}s → {RATE_DELAY_SUSTAINED}s after {
        RATE_WARMUP_SECONDS:.0f}s"
    print(f"  {CYAN}Rate:{RESET}       {rate_info}")
    if manifest:
        print(
            f"  {CYAN}Manifest:{RESET}  {
                manifest.project_id} ({RAGCONFIG_PATH})")
    print(f"  {DIM}{'─' * 60}{RESET}")
    print()
...
def _print_sync_plan(
    ...
    print(
        f"    {
            _status_icon('updated')} {YELLOW}{n_updated}{RESET} updated files")
    if n_incomplete > 0:
        print(
            f"    {
                _status_icon('incomplete')} {YELLOW}{n_incomplete}{RESET} incomplete (interrupted uploads)")
    print(
        f"    {
            _status_icon('unchanged')} {DIM}{n_unchanged}{RESET} unchanged (skipped)")
    print(
        f"    {
            _status_icon('stale')} {RED}{
            len(stale_docs)}{RESET} stale (will delete)")
    print(f"  {DIM}{'─' * 50}{RESET}")
    ...
    if to_ingest:
        print()
        shown = 0
        for fp, rel, _h, action in to_ingest[:20]:
            icon = _status_icon(
                "new") if action == "new" else _status_icon("updated")
            ct = detect_content_type(fp)
    ...
    if total_action == 0:
        ...
        print(
            f"  {BOLD}{len(to_ingest)}{RESET} files to embed, "
...
def _print_store_status(
    ...
    for doc_path, info in sorted(existing.items()):
        ...
        if doc_path in local_by_docpath:
            ...
            status = "unchanged" if local_hash == h else "updated"
        else:
            status = "stale"
    ...
    print(
        f"  {BOLD}{
            len(existing)}{RESET} documents, {BOLD}{total_chunks}{RESET} total chunks")
    print()
    ...
    if missing:
        ...
        print(
            f"  {YELLOW}{
                len(missing)} local file(s) not yet in store:{RESET}")
...
def _print_summary(
    ...
    print(
        f"  {BOLD}Upload Complete{RESET}  {DIM}{
            _duration_fmt(elapsed)}{RESET}")
    print(f"  {BOLD}{'═' * 60}{RESET}")
    print()
    ...
    if tracker.chunks_ok > 0:
        ...
        print(
            f"    {DIM}{
                rate:.1f} chunks/sec · {
                _size_fmt(
                    tracker.bytes_processed)} processed{RESET}")
...
def _print_failure_report(
    ...
    print(
        f"  {RED}{BOLD}⚠ Upload Errors ({
            len(failed_files)} file{
            's' if len(failed_files) != 1 else ''}){RESET}")
    print(f"  {DIM}{'─' * 50}{RESET}")
    ...
    for fpath, ok, total, errs in failed_files:
        status = f"{RED}✗ 0/{total}{RESET}" if ok == 0 else f"{YELLOW}◐ {ok}/{total}{RESET}"

        ct = detect_content_type(fpath)
        badge = _type_badge(ct, detect_language(fpath))
        print(f"    {status}  {badge} {CYAN}{fpath}{RESET}")

        unique_errs = sorted({e for e in errs if e != "aborted"})
    ...
    if dirs_seen:
        ...
        for d, count in sorted(dirs_seen.items(), key=lambda x: -x[1]):
            print(
                f"      {DIM}{d}/{RESET} ({count} file{'s' if count != 1 else ''})")

    print()

    # Retry hint
    print(
        f"  {DIM}Tip: re-run with --sync to retry only failed/missing files{RESET}")
...
def _print_upload_stats(files: list[str], repo_root: str) -> int:
    ...
    if total_chunks > 0:
        ...
        print()
        print(
            f"    {DIM}Estimated time: ~{_duration_fmt(est_secs)} "
...
def _start_timer_thread(
    ...
    """Background thread for elapsed time, ETA, and rate-limit pause
    display."""
    ...
    def _timer_loop() -> None:
        ...
        while not stop_event.wait(1):
            ...
            if pause_until > now:
                ...
                with _print_lock:
                    sys.stdout.write(
                        f"\r  {YELLOW}{BOLD}{pause_label}: {
                            remaining:>3}s  {bar}{RESET}   ")
                    sys.stdout.flush()
...
def _compute_sync_plan(
    ...
    """Compare local files against store.

    Returns (to_ingest, stale_docs, counters).
    ...
    for rel, fp in local_by_docpath.items():
        ...
        if info is None:
            ...
            if ext in CODE_EXTENSIONS or ext in CONFIG_EXTENSIONS:
                current_limit = (
                    CODE_CHUNK_MAX_CHARS if ext in CODE_EXTENSIONS else CONFIG_CHUNK_MAX_CHARS)
            else:
                current_limit = MAX_CHUNK_CHARS
            stored_max: int | None = info.get(
                "max_chunk_chars")  # type: ignore[assignment]
            ...
            stored: int | None = info.get(
                "chunk_count")  # type: ignore[assignment]
...
Examples:
  ...
  pgvector-upload /path/to/project --export plan.json # export sync plan
    ...
    repo_root = manifest.project_root if manifest else find_repo_root(
        args.path)
    project_id = (
        manifest.project_id if manifest else os.path.basename(
            repo_root.rstrip(
                os.sep)))
    ...
    if args.sync:
        ...
        to_ingest = [(fp, os.path.relpath(fp, repo_root), "", "full")
                     for fp in files]
        stale_docs = []
    ...
    if args.confirm and sys.stdin.isatty() and verbosity != "json":
        total_action = len(to_ingest) + len(stale_docs)
        print(
            f"  {BOLD}About to process {total_action} action(s). Continue? [y/N]{RESET} ",
            end="",
        )
        try:
    ...
    if stale_docs:
        for doc_path in stale_docs:
            ...
            if verbosity not in ("json", "compact"):
                safe_print(
                    f"  {
                        _status_icon('stale')} {DIM}{doc_path}{RESET} (deleted {deleted} chunks)")
        ...
        for fp, rel, h, action in to_ingest:
            if action in ("updated", "forced", "incomplete", "oversized"):
                ...
                if verbosity not in ("json", "compact"):
                    safe_print(
                        f"  {
                            _status_icon('updated')} {CYAN}{rel}{RESET} ({action}, deleted {deleted} old chunks)")
            ...
            for fp, rel, h, action in to_ingest:
                if verbosity == "compact":
                    safe_print(
                        f"  {
                            _status_icon(
                                'new' if action == 'new' else 'updated')} {CYAN}{rel}{RESET}")
    ...
    if _rate_limiter.is_aborted():
        ...
        print(
            f"  Stored {GREEN}{
                tracker.chunks_ok}{RESET}/{
                tracker.chunks_total} chunks before stopping.")
        print(
            f"  {DIM}Remaining chunks were skipped to protect your account.{RESET}")
        print()
        if tracker.failed_files:
            _print_failure_report(tracker.failed_files)
        print(
            f"  {DIM}Tip: re-run with --sync to resume where you left off{RESET}")
        print()
        sys.exit(2)
