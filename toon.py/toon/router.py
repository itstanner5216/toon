...
def field_matcher_matches(
    ...
    if matcher.field_path is not None and field_path != matcher.field_path:
    ...
    if matcher.min_length is not None and isinstance(value, str):
    ...
    if matcher.max_length is not None and isinstance(value, str):
...
if __name__ == "__main__":
    ...
    print(
        f"  route_field('payload_extra', ...) → {
            action5!r}  ✓  (no false prefix)")
    ...
    print(
        f"  route_field('field', 42) → {
            action9!r}  ✓  (non-str skips length)")
    ...
    assert action11 == "passthrough", f"Expected 'passthrough', got {
        action11!r}"
    print(
        f"  route_field(..., enabled=False) → {
            action11!r}  ✓  (kill-switch)")
    ...
    assert action12 == "passthrough", f"Expected 'passthrough', got {
        action12!r}"
    print(
        f"  route_field('payload.output', 'tiny') → {
            action12!r}  ✓  (AND: length fails)")
    ...
    print(
        f"  route_field('payload.output', <100 chars>) → {
            action13!r}  ✓  (AND: both pass)")

    # String is long enough but pattern doesn't match → no match
    action14 = route_field("payload.input", "x" * 100, cfg6)
    assert action14 == "passthrough", f"Expected 'passthrough', got {
        action14!r}"
    print(
        f"  route_field('payload.input', <100 chars>) → {
            action14!r}  ✓  (AND: pattern fails)")
    ...
    assert action15 == "passthrough", f"Expected 'passthrough', got {
        action15!r}"
