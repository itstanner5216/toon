...
from ._utils import NORMALIZERS, blake2b_hash, canonical_json, normalize_value
...
class Deduplicator:
    ...
    def deduplicate(self, entries: list[Any]) -> DedupResult:
        ...
        collapsed, template_removed = self._collapse_templates(
            unique, templates)
    ...
    def _detect_templates(
            self, entries: list[dict]) -> dict[str, TemplateInfo]:
        ...
        """
        templates: dict[str, TemplateInfo] = {}
        for _schema_h, indices in self._schema_groups.items():
            if len(indices) < 3:
                continue
    ...
    def _compute_template_key(group: list[dict]) -> str | None:
        ...
        for key in keys:
            values = [str(e["content"].get(key))
                      for e in group if isinstance(e["content"], dict)]
            if not values:
                continue
        ...
        for info in templates.values():
            ...
            for e in entries:
                ...
                if orig_idx == info.first_index or orig_idx == info.last_index:
                    continue
                if e.get(
                        "template_id") and info.first_index < orig_idx < info.last_index:
