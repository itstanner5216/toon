...
from ._utils import compute_gini, estimate_tokens_obj, find_kneedle, flatten_to_text, pearson_r
from .budget import BudgetAllocation, BudgetAllocator
...
class CompressConfig:
    ...
    # Budget parameters
    tier_ratios: dict[str, float] = field(
...
def _score_entries(
    ...
    chunks = [{"chunk_id": str(i), "text": texts[i]} for i in range(n)]
    index = BMXPlusIndex()
    ...
    if n <= _GRAPH_LIMIT:
        ...
        sage_result = sage.rank_sentences(
            [texts[i] for i in sample_indices],
            top_k=sample_size,
        )
        sample_scores = {
            sample_indices[i]: sage_result.scores[i] for i in range(sample_size)}
        ...
        core_query = " ".join(texts[idx][:200]
                              for idx, _ in sorted_sample[:sample_core_size])
        core_query = query + " " + \
            core_query[:500] if query else core_query[:2000]
        ...
        for i in range(n):
            ...
            if i in sample_scores:
                sage_norm = sample_scores[i] / \
                    max_sage if max_sage > 0 else 0.0
    ...
    combined_query = query + " " + \
        core_text[:500] if query else core_text[:2000]
    ...
    if r > config.redundancy_r_threshold:
        logger.info(
            ...
            r,
            config.redundancy_r_threshold,
        )
        final_scores = list(self_scores)
    else:
        final_scores = [
            0.4 * ss + 0.6 * rs for ss,
            rs in zip(
                self_scores,
                relevance_scores,
                strict=False)]
...
def _compress_entries(
    ...
    original_tokens = sum(estimate_tokens_obj(
        e["content"]) for e in scored.entries)

    return CompressedOutput(
        entries=compressed,
        budget_used=total_tokens,
        stats={
            "compression_ratio": round(1.0 - total_tokens / max(1, original_tokens), 4),
...
def _compress_entry(
    ...
    if isinstance(content, str):
        ...
        if config.sagerank_top_k > 0 and len(
                content) > budget and len(content) > 500:
            sr = SageRank()
...
if __name__ == "__main__":
            ...
            "data": [
                {
                    ...
                    "created_at": f"2026-04-{10 + i}T12:00:00Z",
                    "role": "member",
                }
                for i in range(20)
            ],
            ...
            "data": [
                {
                    ...
                    "name": f"User {i}",
                    "email": f"user{i}@test.com",
                    "created_at": f"2026-05-{10 + i}T14:30:00Z",
                    "role": "member",
                }
                for i in range(20)
            ],
    ...
    print(
        f"Compression: {100 * (1 - len(result_json) / len(original_json)):.1f}%")
    ...
    for i in range(10000):
        ...
        if kind == 0:
            ...
            entries_10k.append(f"2026-04-12T{i %
                                             24:02d}:{i %
                                                      60:02d}:00Z INFO Processing item {i}")
        elif kind == 2:
            entries_10k.append(
                {
                    "error": None,
    ...
    print(
        f"  10,000 entries → {
            len(result_10k)} entries in {
            elapsed_10k:.2f}s")
    assert elapsed_10k < 10.0, f"Performance target missed: {
        elapsed_10k:.2f}s > 10s"
