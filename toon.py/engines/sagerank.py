...
class SageRank:
    ...
    def _compute_eidf(
        ...
        for term, df in doc_freqs.items():
            ...
            if blend_alpha < 0.001:
                ...
                norm_ent = min(
                    entropy / max_ent,
                    1.0) if max_ent > 0.0 else 0.0
                shannon_info = max(1.0 - norm_ent, 0.0)
                info[term] = blend_alpha * shannon_info + \
    ...
    def _build_graph(
        ...
        def _tf_sat(tf: int, dl: int) -> float:
            if avg_dl <= 0:
                return float(tf)
            return (tf * (k1 + 1.0)) / (tf + k1 * (1.0 - b + b * dl / avg_dl))
        ...
        norms = {
            i: math.sqrt(v) if v > 0.0 else 1.0 for i,
            v in norm_sq.items()}
        ...
        for term, posting in posting_lists.items():
            ...
            weighted = [(idx, e * _tf_sat(tf, doc_lengths[idx]))
                        for idx, tf in items]
    ...
    def _position_prior(centrality: list[float], n: int) -> list[float]:
        ...
        for i in range(n):
            ...
            weights.append(1.0 + lead_strength * lead + trail_strength * trail)
        return weights
    ...
    def _pagerank(
        ...
        p_sum = sum(p)
        p = [v / p_sum for v in p] if p_sum > 0 else [1.0 / n] * n
        ...
        for i in range(n):
            ...
            out_total[i] = total
            if total > 0:
                out_edges[i] = [(j, w / total) for j, w in neighbours.items()]
    ...
    def _extract_with_coverage(
        ...
        while len(selected) < top_k and remaining:
            ...
            for idx in remaining:
                ...
                if total_w > 0.0:
                    novel_w = sum(
                        w for t, w in sent_weights[idx].items() if t not in covered)
    ...
    def rank_sentences(
        ...
        posting_lists, doc_freqs, doc_lengths, avg_dl = self._build_posting_lists(
            sent_tokens)
        ...
        centrality = [sum(adjacency.get(i, {}).values()) for i in range(n)]
        ...
        stats = {
            ...
            "lead_bias": round(self._position_prior(centrality, n)[0] - 1.0, 4)
            if n > 0
            else 0.0,
    ...
    def extract_keywords(
        ...
        if not sentences:
            return []
        sent_tokens = [self._tokenize(s) for s in sentences]
        posting_lists, doc_freqs, _, _ = self._build_posting_lists(sent_tokens)
        n = len(sentences)
